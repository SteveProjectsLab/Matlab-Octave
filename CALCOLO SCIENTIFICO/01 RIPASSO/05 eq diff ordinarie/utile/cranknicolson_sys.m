function [tn,un]=cranknicolson_sys(odefun,tspan,y0,Nh, varargin)
% cranknicolson_sys: metodo di Crank Nicolson per l'appx di sistemi di edo
%                 richiede broyden.m
%  [tn,un]=cranknicolson_sys(odefun,tspan,y0,Nh)
%  [tn,un]=cranknicolson_sys(odefun,tspan,y0,Nh,tol)
%  [tn,un]=cranknicolson_sys(odefun,tspan,y0,Nh,tol,kmax)
%
...
for n=1:Nh
    ...
    wn=un(n,:).';
    r=@(x)x-wn -h/2*(odefun(tn(n),wn)+odefun(tn(n+1),x));
