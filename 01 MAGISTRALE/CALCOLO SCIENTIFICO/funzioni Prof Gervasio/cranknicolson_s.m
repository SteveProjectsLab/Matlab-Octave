function [tn,un]=cranknicolson_s(odefun,tspan,y0,Nh, varargin)
...
%   condizione iniziale y0 usando il metodo di Crank
%   Nicolson su una griglia equispaziata di Nh
%   intervalli.
...
%   Altrimenti
%   [tn,un]=cranknicolson_s(odefun,tspan,y0,Nh,tol,nmax)
...
for n=1:Nh
    wn=un(n,:).';
    r=@(x)x-wn-h2*(odefun(tn(n),wn)+odefun(tn(n+1),x));
    [z]=broyden(r,B0,wn,tol,nmax,pflag);
    un(n+1,:)=z.';
end
