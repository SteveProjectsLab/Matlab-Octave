...
%-------------------------------------------------------------------------
fprintf('\nES2 Punto 1\n\n');
fprintf("Calcolo le trasformata di Fourier di alcuni segnali noti")
%-------------------------------------------------------------------------
x1 = rect(t);
X1 = T_Fourier(x1,t,f);
...
figure('name','Trasformate note','Position',[0 0 scrsz(3) scrsz(4)-80]);
...
%-------------------------------------------------------------------------
fprintf('\nES2 Punto 2\n\n');
fprintf("grafico della trasformata di cos (4 pi t)..")
%-------------------------------------------------------------------------
...
figure('name','Trasformata del cos','Position',[0 0 scrsz(3) scrsz(4)-80]);
...
%-------------------------------------------------------------------------
fprintf('\nES3 Punto 1\n\n');
fprintf("Teorema della risposta in frequenza\n ")
fprintf("Dato un sistema LTI e una h(t) disegnare modulo e fase di H(f)\n ")
%-------------------------------------------------------------------------
...
H = T_Fourier(h,t,f);
figure('name','modulo e fase della risposta in frequenza','Position',[0 0 scrsz(3) scrsz(4)-80]);
plot(f,abs(H),'b'); title('$\vert H_1(f)\vert$','Interpreter','latex','FontSize',15); grid on;
figure('name','modulo e fase della risposta in frequenza','Position',[0 0 scrsz(3) scrsz(4)-80]);
plot(f,unwrap(angle(H)),'r'); title('$<H_1(f)$','Interpreter','latex','FontSize',15); grid on;
...
%-------------------------------------------------------------------------
fprintf('\nES3 Punto 2\n\n');
fprintf("Dato un segnale in ingresso calcolare l'0scita per varie frequenze\n ")
%-------------------------------------------------------------------------
...
for n = 1:close length(fvet)
    ...
    plot(t,y(n,:));
    title(sprintf(['$$y_' ...
        '%d(t)$$'],n),'Interpreter','latex','FontSize',15); grid on;
...
%-------------------------------------------------------------------------
fprintf('\nES4 Punto 1\n\n');
fprintf("verifica della proprietà di traslazione\n")
%-------------------------------------------------------------------------
...
scrsz = get(0,'ScreenSize');
figure('name','Traslazioni, real/imaginary','Position',[0 0 scrsz(3) scrsz(4)-80]);
...
figure('name','Traslazioni, abs/phase','Position',[0 0 scrsz(3) scrsz(4)-80]);
...
figure('name','Verifica convoluzione','Position',[0 0 scrsz(3) scrsz(4)-80]);
...
%-------------------------------------------------------------------------
fprintf('\nES5 Punto 1\n\n');
fprintf("visualizzare la risposta in frequenzadell'impulso del filtro ideale LP, finestrata con rect diversi\n")
%-------------------------------------------------------------------------
...
scrsz = get(0,'ScreenSize');
figure('name','Gibbs phenomenon, window: rect(t/4)','Position',[0 0 scrsz(3) scrsz(4)-80]);
...
figure('name','Gibbs phenomenon, window: rect(t/10)','Position',[0 0 scrsz(3) scrsz(4)-80]);
...
figure(['n' ...
    'ame'],'Gibbs phenomenon, window: rect(t/20)','Position',[0 0 scrsz(3) scrsz(4)-80]);
