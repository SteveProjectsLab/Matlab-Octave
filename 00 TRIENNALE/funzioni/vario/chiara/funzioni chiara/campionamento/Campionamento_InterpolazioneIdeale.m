function x_ideale= Campionamento_InterpolazioneIdeale(x,estremo_inf,passo,estremo_sup,t,f, numero_figure)
%CAMPIONAMENTO_IDEALE Summary of this function goes here
...
for n= estremo_inf:passo:estremo_sup
    ...
    %INTERPOLAZIONE IDEALE: x_ideale= x_camp*sinc(t/T_campionam.)
    
    x_ideale=zeros(1, length(t));
    
    for k=1:length(t_sample)
        
        x_ideale= x_ideale + x_campionato(t_sample(k))*sinc((t-t(t_sample(k)))/T_campionamento);
    end
    
    subplot(r,c,3),plot(t, x_ideale, 'b', t, x,'black'),title (sprintf('Segnale ricostruito idealmente, con T_campionamento=%.2f ', T_campionamento))
     
    fprintf('Energia dell''errore di ricostruizione con interpolazione ideale e Tc= %.3f: %.3f\n',T_campionamento, integrale(abs(x-x_ideale).^2,dt))
